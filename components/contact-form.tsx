...
export function ContactForm() {
  ...
  const selectedEngagement = getEngagement(searchParams.get("engagement"));
  return <form action={action} className="space-y-5">
    {selectedEngagement ? <div className="rounded-lg border border-primary/25 bg-primary/10 px-4 py-3"><div className="font-mono text-[9px] uppercase tracking-[0.18em] text-primary">Selected engagement</div><p className="mt-1 text-sm font-medium">{selectedEngagement.title} <span className="font-normal text-muted-foreground">· {selectedEngagement.priceLabel}</span></p></div> : null}
    ...
    <div className="grid gap-5 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="company">Company <span className="text-muted-foreground">(optional)</span></Label><Input id="company" name="company" autoComplete="organization" placeholder="Company or shop" /></div><div className="space-y-2"><Label htmlFor="projectType">What are we building?</Label><select key={selectedEngagement?.id ?? "unselected"} id="projectType" name="projectType" defaultValue={selectedEngagement?.title ?? ""} className="h-11 w-full rounded-lg border border-border bg-background/70 px-3 text-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/15"><option value="" disabled>Select a project type</option>{engagements.map((engagement) => <option key={engagement.id}>{engagement.title}</option>)}<option>Custom web application</option><option>Workflow automation</option><option>Website or client portal</option><option>CNC / signage system</option><option>Prototype or consulting</option><option>Something else</option></select></div></div>
