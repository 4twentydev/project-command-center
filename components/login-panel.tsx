...
export function LoginPanel({ nextPath = "/dashboard", allowSetup = false }: { nextPath?: PrivateDestination; allowSetup?: boolean }) {
  ...
  return <Card className="w-full max-w-md border-border/70 bg-card/85 shadow-2xl shadow-cyan-950/20 backdrop-blur-xl"><CardContent className="p-7 sm:p-8">
    ...
    {allowSetup ? <button className="mt-5 w-full text-center text-xs text-muted-foreground transition hover:text-foreground" onClick={() => { setMode(mode === "login" ? "setup" : "login"); setMessage(""); }}>{mode === "login" ? "First visit? Create the owner account" : "Already configured? Return to sign in"}</button> : null}
  </CardContent></Card>;
}
