...
export function ConsultationWorkspace({ initialConsultations, initialLead, storageAvailable }: { initialConsultations: ConsultationRecord[]; initialLead: ContactInquiry | null; storageAvailable: boolean }) {
  ...
  async function save() {
    ...
    try {
      ...
      setConsultations((current) => [saved, ...current.filter((item) => item.id !== saved.id)]);
